...
### Part A
We calculate the expected value of $\tilde{\beta}$ as follows:
\begin{align}
E(\tilde{\beta}) &= E[(X^TX+\lambda I)^{-1} X^T Y] \\
                 &= (X^TX+\lambda I)^{-1} X^T E(Y) \\
               Y &= X\beta + e \\
             E(Y)&= E(X\beta + e) \\
                 &= X\beta \\
E(\tilde{\beta}) &= (X^TX+\lambda I)^{-1} X^T X \beta
\end{align}
...
\begin{align}
Bias^2 &= E(y_0 - \tilde{f}(x_0))\\
&= x_0^T \beta - x_0^T E(\beta)\\
MSE(Y-\hat{Y}) &= Var(\hat{Y}) + Bias^2(\hat{Y}) + Var(\epsilon) \\
               &= Var(\vec{x}_0^T \tilde{\beta}) + Bias^2(\vec{x}_0^T \tilde{\beta}) + Var(\epsilon)\\
               &= W \sigma^2 W^T + Bias^2(\vec{x}_0^T \tilde{\beta}) + \sigma^2 \\
...
i. Below is Bert-Ernie's corrected code. His mistakes were that he should have used a prediction interval instead of a confidence interval. He also should have used a 90% interval instead of a 0.975% to ensure a 95% certainty for the lower limit.
```{r}
...
ii. Analytical expression for the prediction interval: 
$$
...
Now we can implement this interval in R analytically, noting that a 90% prediction interval gives a 95% prediction for our lower bound.
```{r}
...
i.
Here we fit a QDA model and note that 441 observations were classified in the "A" category. 299 of these were correct identifications. 
```{r}
...
bigfoot_logistic_test_pred <- bigfoot_logistic_test_probs > .5
table(bigfoot_logistic_test_pred, test$class)
```

ii. We multiply the odds by `r exp(coef(bigfoot_logistic )['sawTRUE'])`  (option 4)
...
bigfoot_qda_class <- predict(bigfoot_qda, test)$class
table(bigfoot_qda_class, test$class)

```

We note that 447 observations in the test set were identified as class "A", of which 389 were correct identifications. 
...
table(bigfoot_knn_pred, test$class)
```
